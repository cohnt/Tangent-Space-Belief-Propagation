...
def pairwiseDistErr(embedded_points, true_parameters, dist_metric="l2", mat_norm="fro"):
	embedded_dists = pairwise_distances(normalize(embedded_points, axis=1, copy=False), metric=dist_metric, n_jobs=-1)
	true_dists = pairwise_distances(normalize(true_parameters, axis=1, copy=False), metric=dist_metric, n_jobs=-1)
